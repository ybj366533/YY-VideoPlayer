...
public class IndicatorSeekBar extends View implements ViewTreeObserver.OnGlobalLayoutListener, ViewTreeObserver.OnScrollChangedListener {
    ...
    private IndicatorSeekBarListener listener;

    private List<Float> mSplitList = new ArrayList<>();

    // 热点设置
    public List<Float> getmSplitList() {
        return mSplitList;
    }

    public void setmSplitList(List<Float> mSplitList) {
        this.mSplitList = mSplitList;
    }

    public void setIndicatorSeekBarListener(IndicatorSeekBarListener listener) {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        p.mCacheProgressTrackColor = ta.getColor(R.styleable.IndicatorSeekBar_isb_track_progress_bar_color, p.mCacheProgressTrackColor);
    ...
    private void calculateCacheProgressTouchX() {
        //progress
        float touchX = (p.mCacheProgress - p.mMin) * mSeekLength / (p.mMax - p.mMin) + mPaddingLeft;
        calculateCacheTouchX(touchX);
    }

    float getCacheTouchX() {
        calculateCacheProgressTouchX();
        return mCacheTouchX;
    }
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        if (!mDrawAgain) {
            ...
            float touchX = (p.mProgress - p.mMin) * mSeekLength / (p.mMax - p.mMin) + mPaddingLeft;
            calculateTouchX(touchX);
            float cacheX = (p.mCacheProgress - p.mMin) * mSeekLength / (p.mMax - p.mMin) + mPaddingLeft;
            calculateCacheTouchX(cacheX);
            mDrawAgain = true;
        }
        float thumbX = getThumbX();
        float cacheX = getCacheX();
        //draw BG track
        ...
        mStockPaint.setColor(p.mCacheProgressTrackColor);
        mStockPaint.setStrokeWidth(p.mCacheProgressTrackSize);

        canvas.drawLine(mSeekStart, mTrackY, cacheX, mTrackY, mStockPaint);
        ...
        for (int x = 0; x < mSplitList.size(); x++) {
            float item = mSplitList.get(x) * mSeekEnd;
//            Log.e("mSplitList", "mSplitList-->ITEM = " + item);
            canvas.drawLine(item, mTrackY, item + 2, mTrackY, mStockPaint);
        }
        //draw tick
    ...
    private float getThumbX() {
        ...
        if (mThumbXCache <= mSeekStart) {
            ...
            Log.e("mThumbXCache", "mThumbXCache = " + mThumbXCache);
    ...
    private float getCacheX() {
        float mThumbXCache = mCacheTouchX - p.mBackgroundTrackSize / 2.0f;
        float mThumbX;
        if (mThumbXCache <= mSeekStart) {
            if (mThumbXCache <= mPaddingLeft) {
                mThumbX = getPaddingLeft() - p.mBackgroundTrackSize / 2.0f;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                listener.onProgressChanged(p.mProgress);
                break;
...
//                Log.e("mProgress", p.mProgress + "#");
                listener.onSeekTo(p.mProgress);
                mIsTouching = false;
                invalidate();
                if (p.mShowIndicator) {
                    mIndicator.hide();
    ...
    private void calculateCacheTouchX(float touchX) {
        int touchBlockSize = Math.round((touchX - mPaddingLeft) / mSeekBlockLength);
        mCacheTouchX = mSeekBlockLength * touchBlockSize + mPaddingLeft;
    }
    ...
    public int getCacheProgress() {
        return Math.round(p.mCacheProgress);
    }
    ...
    /**
     * Sets the current cache progress to the specified value.
     *
     * @param progress a new progress value , if the new progress is less than min , it will set to min ,if over max ,will be max.
     */
    public synchronized void setSecondaryProgress(float progress) {
        if (progress < p.mMin) {
            p.mCacheProgress = p.mMin;
        } else if (progress > p.mMax) {
            p.mCacheProgress = p.mMax;
        } else {
            p.mCacheProgress = progress;
        }
//        setListener(false);
        float touchX = (p.mCacheProgress - p.mMin) * mSeekLength / (p.mMax - p.mMin) + mPaddingLeft;
        calculateCacheTouchX(touchX);
    ...
    public static class Builder {
        ...
        public Builder setCacheProgressTrackColor(@ColorInt int progressTrackColor) {
            p.mCacheProgressTrackColor = progressTrackColor;
            return this;
        }
